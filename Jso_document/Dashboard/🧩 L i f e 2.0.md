...
`$=dv.list(dv.pages('"00 - Notes"').sort(f=> f.file.mtime.ts,"desc").limit(10).file.link)`

```button
name 📝 Add note
type note(NewNote, split) template
action NoteTemplate
templater true
class add-btns
```
...
<iframe src="https://flocus.com/online-flip-clock/" width="100%" height="500px"></iframe>
